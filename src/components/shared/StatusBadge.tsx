...
import type { ObligationStatus, PaymentStatus } from '@/types'
import { cn } from '@/lib/utils'

const statusConfig: Record<PaymentStatus | ObligationStatus, { label: string; variant: string; className: string }> = {
  ...
  calculated: { label: 'Рассчитано', variant: 'default', className: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 border-slate-200 dark:border-slate-700' },
  due: { label: 'К оплате', variant: 'default', className: 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200 border-amber-200 dark:border-amber-800' },
}

export function StatusBadge({ status }: { status: PaymentStatus | ObligationStatus }) {
